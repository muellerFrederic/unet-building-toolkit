...
class ModelCreator(abc.ABC):
    ...
    def create_model(self, input_shape, filter_base=64, filter_direction=1, depth=5, num_convolutions=2, num_classes=2,
...
class UnetCreator(ModelCreator):
    def _create_network_core(self, input_data, filters, depth, num_convolutions, num_classes, downsampling, upsampling,
                             block_contracting, block_expanding, batch_normalization, **kwargs):
        if downsampling is None:
            downsampling = wrappers.MaxPoolingWrapper()

        if upsampling is None:
            upsampling = wrappers.TransposedConvolutionWrapper()
...
class VnetCreator(ModelCreator):
    def _create_network_core(self, input_data, filters, depth, num_convolutions, num_classes, downsampling, upsampling,
                             block_contracting, block_expanding, batch_normalization, **kwargs):
        if downsampling is None:
            downsampling = wrappers.DownConvolutionWrapper()
...
class MrUnetCreator(ModelCreator):
    pass


class UnetPlusPlusCreator(ModelCreator):
    pass
